...
{	
	...
	pr_debug("Macle ant_state_show state = %d, custome_state=%d\n", global_ant_info->ant_check_state, state);
...
{	
	...
	if (rc) {
		pr_debug("Macle kstrtoint failed.rc = %d\n", rc);
		return rc;
	}
...
static int ant_probe(struct platform_device *pdev)
{
	...
	pr_debug("Macle ant_probe\n");
		
	if (pdev->dev.of_node) {
		ant_info = kzalloc(sizeof(struct ant_check_info), GFP_KERNEL);		  
		if (!ant_info)
			pr_debug("Macle %s: failed to alloc memory for module data\n",__func__);
			return -ENOMEM;
	...
	if (!ant_info->ipdev) {
		pr_debug("ant_probe: input_allocate_device fail\n");
		goto input_error;
	}
	...
	if (rc) {
		pr_debug("ant_probe: input_register_device fail rc=%d\n", rc);
		goto input_error;
	}
	...
	if (gpio_is_valid(ant_info->irq_gpio)) {
...
#endif
		ant_interrupt, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "ant-switch-irq", ant_info);
		...
		pr_debug("Macle irq gpio not provided\n");
	        goto free_input_device;
	}

	pr_debug("ant_probe end\n");
